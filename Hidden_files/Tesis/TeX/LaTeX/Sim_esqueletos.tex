...
Considerando que ya se generaron las matrices $D'$ (ver Sección \ref{SimDemandaAlumnos}) y \textit{mat\_solicitudes} (ver Sección \ref{SimSolicitudesProfesores}), el proceso que seguimos para obtener un esqueleto es el siguiente:
...
\section{Obtención de mat\_esqueletos} \label{sec_esqueletos}
%el Modelo de Mezcla Gaussiana visto en la sección \ref{sec_GMM}.
En esta sección vamos a explicar cómo generamos la matriz \textit{mat\_esqueleto}, utilizando la metodología $B$ seleccionada en la Sección \ref{GMM_D}. La matriz tiene $t$ renglones y $m$ columnas. En la entrada $(h,j)$ tiene el número de grupos simulados para la hora $h$ y la materia $j$. La matriz \textit{mat\_esqueleto} depende de la demanda de alumnos y de las solicitudes de los profesores.
