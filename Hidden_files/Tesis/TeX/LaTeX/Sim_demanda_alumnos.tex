...
En \textit{R} se puede obtener dicha distribución con la función \verb@normalmixEM()@. La función regresa un objeto de tipo \textit{mixEM} el cual contiene el modelo de una mezcla de distribuicones normales obtenidas por el algoritmo de maximización de la esperanza (EM - \textit{expectation maximization algorithm}).
...
A continuación vamos a explicar los principales parámetros que recibe la función \verb@normalmixEM()@.

\begin{itemize}
\item[-] \textit{x: } Vector con los datos a los que se quiere aplicar el modelo.

\item[-] \textit{mu: } Vector con las medias iniciales para el algoritmo EM.

\item[-] \textit{k: } Número de normales que se ajustan a los datos.
\end{itemize}


A continuación vamos a explicar los elementos del objeto de tipo \textit{mixEM} que utilizamos para poder simular la demanda de alumnos en la siguiente sección.

\begin{itemize}
\item[-] \textit{mu: } Vector con las medias finales del modelo.
